...
#TODO добавить еще время реагирования разница между созданным вызовом и принятым погрузчиком
#TODO пилим дашборд
#TODO каолендарь генерировать полносттью на python ВКЛЮЧАЯ создание таблицы, прописывая типы данных на уровне pandas

#TODO некорректно приджойниваются погрузчики либо инфа в таблице измерений не актуальная. P.S. многие полдьзователи отсутсвуют в pa_oper
...
FROM
	...
	'public')
where
	tg_id is not null AND tg_id NOT IN (5773698501, 325813539);
...
--- основное вычисление количества минут для ПЕРВОЙ смены. Делаем вьюшку
...
------------------------------
--- сформируем первую витрину в разрезе дня


--create table cdm.loader_timer engine=MergeTree order by period_date as 
SELECT
